...
SET(NVidiaUSD_LIB_NAMES_SHARED ar arch gf js kind pcp plug sdf tf trace usd usdGeom vt work usdShade usdLux usdSkel python37.lib)
...
FIND_PATH(NVidiaUSD_INCLUDE_DIR_RELEASE
    NAMES "pxr/pxr.h"
    PATHS
    ${NVidiaUSD_ROOT}/release/include
    ${NVidiaUSD_ROOT}
    /usr/local/include
    /usr/include
    NO_DEFAULT_PATH
...
FIND_PATH(NVidiaUSD_INCLUDE_DIR_DEBUG
    NAMES "pxr/pxr.h"
    PATHS
    ${NVidiaUSD_ROOT}/debug/include
    ${NVidiaUSD_ROOT}
    /usr/local/include
    /usr/include
    NO_DEFAULT_PATH
...
if (EXISTS ${NVidiaUSD_INCLUDE_DIR_RELEASE})
    foreach(_usd_comp MAJOR MINOR PATCH)
        file(STRINGS
            "${NVidiaUSD_INCLUDE_DIR_RELEASE}/pxr/pxr.h"
            _usd_tmp
...
FIND_PATH(NVidiaUSD_LIBRARY_DIR_RELEASE
    NAMES "usd.lib"
    PATHS
    ${NVidiaUSD_ROOT}/release/lib
    ${NVidiaUSD_ROOT}
    /usr/local/include
    /usr/include
    NO_DEFAULT_PATH
...
FIND_PATH(NVidiaUSD_LIBRARY_DIR_DEBUG
    NAMES "usd.lib"
    PATHS
    ${NVidiaUSD_ROOT}/debug/lib
    ${NVidiaUSD_ROOT}
    /usr/local/include
    /usr/include
    NO_DEFAULT_PATH
...
foreach(dl rl IN ZIP_LISTS NVidiaUSD_LIB_NAMES_DEBUG NVidiaUSD_LIB_NAMES_RELEASE)
  unset(_nvusd_dl_name CACHE)
  unset(_nvusd_rl_name CACHE)

  FIND_LIBRARY(_nvusd_dl_name
    NAMES ${dl}
    PATHS
      ${NVidiaUSD_LIBRARY_DIR_DEBUG}
  )

  FIND_LIBRARY(_nvusd_rl_name
    NAMES ${rl}
    PATHS
      ${NVidiaUSD_LIBRARY_DIR_RELEASE}
  )

  list(APPEND NVidiaUSD_LIBRARIES debug ${_nvusd_dl_name} optimized ${_nvusd_rl_name})
